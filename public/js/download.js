...
const renderOverlayErrorMessage = (msg, secondary_msg = null) => {
    ...
    child.innerHTML += `<h1 class="text-center text-lg text-blue-300 hover:text-blue-400"><a href="https://tiktokdownloader.mataberita.com">return to home page</a></h1>`;
...
const waitForBlobToDownload = (blobKey, button) => {
    ...
    if (!BLOBS.hasOwnProperty(blobKey)) {
        ...
        let filename =
            DATA["author"]["unique_id"] + "___" + DATA["title"] + "___";
        if (blobKey == "video") {
            filename += "NO_WM";
            var extension = ".mp4";
